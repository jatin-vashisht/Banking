import {
  ...
  import { cn, formatAmount, formatDateTime, getTransactionStatus, removeSpecialCharacters, convertToINR } from "@/lib/utils"
      ...
      <Table>
        ...
        <TableBody>
          {transactions.map((t: Transaction) => {
            ...
            const amount = formatAmount(convertToINR(t.amount))
            ...
            return (
              <TableRow key={t.id} className={`${isDebit || amount[0] === '-' ? 'bg-[#FFFBFA]' : 'bg-[#F6FEF9]'} !over:bg-none !border-b-DEFAULT`}>
                ...
                <TableCell className={`pl-2 pr-6 whitespace-nowrap font-semibold ${
                  isDebit || amount[0] === '-' ?
                    'text-[#f04438]'
                    : 'text-[#039855]'
                }`}>
